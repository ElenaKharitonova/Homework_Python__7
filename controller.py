...
def start():
    ...
    html = '<html>\n  <head></head>\n  <body>\n'
    html += '    <p {}>name: {} </p>\n'.format(style, elements['name'])
    html += '    <p {}>surname: {} </p>\n'.format(style, elements['surname'])
    html += '    <p {}>phone_number: {} </p>\n'.format(style, elements['phone number'])
    html += '  </body>\n</html>'
    
    with open('phone_numbers.html', 'w') as file:
        file.write(html)
        print('phone_numbers.html')